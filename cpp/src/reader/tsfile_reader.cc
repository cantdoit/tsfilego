...
int TsFileReader::query(std::vector<std::string> &path_list, int64_t start_time,
    ...
    Expression* exp = new storage::Expression(storage::GLOBALTIME_EXPR, time_filter);
    ...
    for (auto path : path_list) {
        ...
        std::string measurement_name = path.substr(last_point_pos + 1, path.size() - last_point_pos); 
        path_list_vec.emplace_back(Path(device_name, measurement_name));
    }
    QueryExpression* query_expression = QueryExpression::create(path_list_vec, exp);
...
std::vector<std::string> TsFileReader::get_all_devices() {
    ...
    if (tsfile_meta != nullptr) {
        ...
        for (const auto &meta_index_entry : tsfile_meta->index_node_->children_) {
...
int TsFileReader::get_timeseries_schema(const std::string &device_id,
    ...
    if (RET_FAIL(tsfile_executor_->get_tsfile_io_reader()
            ->get_device_timeseries_meta_without_chunk_meta(device_id, timeseries_indexs, pa))) {
    } else {
        for (auto timeseries_index : timeseries_indexs) {
            MeasurementSchema ms(timeseries_index->get_measurement_name().to_std_string(),
